    ...
    textPromises.then(parsed => {
      ...
      if (useTriplane) {
        ...
        planePromises.then(planeData => {
          ...
          let planeFeaturesStack =
              new Uint8Array(3 * planeWidth * planeHeight * 4);
          for (let plane_idx = 0; plane_idx < 3; plane_idx++) {
      ...
      for (let occupancyGridIndex = 0;
        ...
        let occupancyGridTexture = createEmptyVolumeTexture(
            w, h, d, THREE.RedFormat, THREE.NearestFilter);
...
function loadOnFirstFrame(
  ...
  if (useSparseGrid) {
    ...
    let uploadFn = (rgbaImage, sliceIndex, volumeWidth, volumeHeight, sliceDepth) => {
      ...
      uploadVolumeSlice(
          rgbPixels, rgbVolumeTexture, sliceIndex, volumeWidth, volumeHeight,
          sliceDepth);
      uploadVolumeSlice(
          densityPixels, densityVolumeTexture, sliceIndex, volumeWidth, volumeHeight,
          sliceDepth);
    };
    let rgbAndDensityVolumeTexturePromise =
        _loadVolumeTextureSliceBySlice(uploadFn, 'rgba');
    ...
    uploadFn = (rgbaImage, sliceIndex, volumeWidth, volumeHeight, sliceDepth) => {
      uploadVolumeSlice(
          rgbaImage, featureVolumeTexture, sliceIndex, volumeWidth, volumeHeight,
          sliceDepth);
    };
    featureVolumeTexturePromise =
        _loadVolumeTextureSliceBySlice(uploadFn, 'feature');

    let allTexturesPromise = Promise.all(
        [rgbAndDensityVolumeTexturePromise, featureVolumeTexturePromise]);
